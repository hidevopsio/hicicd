...
func TestCamelKabob(t *testing.T) {
	for _, tt := range camelKabobs {
		s := crd.CamelCaseToKabobCase(tt.in)
		if s != tt.out {
			t.Errorf("CamelCaseToKabobCase(%q) => %q, want %q", tt.in, s, tt.out)
		}
		u := crd.KabobCaseToCamelCase(tt.out)
		if u != tt.in {
...
func TestConvert(t *testing.T) {
	if _, err := crd.ConvertConfig(model.RouteRule, model.Config{}); err == nil {
		t.Errorf("expected error for converting empty config")
	}
	if _, err := crd.ConvertObject(model.RouteRule, &crd.IstioKind{Spec: map[string]interface{}{"x": 1}}, "local"); err == nil {
	...
	obj, err := crd.ConvertConfig(model.RouteRule, config)
	if err != nil {
		t.Errorf("ConvertConfig() => unexpected error %v", err)
	}
	got, err := crd.ConvertObject(model.RouteRule, obj, "cluster")
	if err != nil {
...
func TestParseInputs(t *testing.T) {
	...
	varr, _, err := crd.ParseInputs("kind: RouteRule\nspec:\n  destination:\n    service: x")
